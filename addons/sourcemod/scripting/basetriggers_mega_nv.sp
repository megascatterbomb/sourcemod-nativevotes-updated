...
#include <sourcemod>
#include <sdktools> // For the GameRules_GetProp call
...
{
	name = "Basic Info Triggers (with mega's nativevotes support)",
	author = "megascatterbomb, AlliedModders LLC",
	description = "Adds ff, timeleft, thetime, and others. Supports mega's fork of nativevotes.",
...
ConVar g_Cvar_MinTimeForMaxRounds;
...
public void OnPluginStart()
{
	...
	g_Cvar_EndOfMapInstantChange = FindConVar("sm_mapvote_instant_change");
	g_Cvar_MinTimeForMaxRounds = FindConVar("sm_mapvote_min_time");
...
void ShowTimeLeft(int client, int who)
{
	...
	bool maxroundscontrolled = false;

	// We set this value directly based on the netprop; can't trust basetrigger's normal tracking.
	// It's only used in this function so should be fine.
	...
	if (notimelimit && g_Cvar_MinTimeForMaxRounds && g_Cvar_MinTimeForMaxRounds.IntValue > 0 && g_Cvar_MaxRounds.IntValue > 0)
	{
		float time = GetGameTime();
		int mintime = g_Cvar_MinTimeForMaxRounds.IntValue;
		int remainingseconds = RoundToFloor((mintime * 60.0) - time);

		int maxrounds = g_Cvar_MaxRounds.IntValue;
		if (g_Cvar_EndOfMapInstantChange && g_Cvar_EndOfMapInstantChange.IntValue)
		{
			maxrounds -= g_Cvar_StartRounds.IntValue;
		}

		if (remainingseconds > 0.0)
		{
			notimelimit = false;
			maxroundscontrolled = true;
			int mins = remainingseconds / 60;
			int secs = remainingseconds % 60;
			written = true;
			FormatEx(finalOutput, sizeof(finalOutput), "%T %d:%02d", "Timeleft", client, mins, secs);

			int remainingrounds = maxrounds - g_TotalRounds;
			int len = strlen(finalOutput);
			if (len < sizeof(finalOutput))
			{ 
				if (remainingrounds > 1)
				{
					FormatEx(finalOutput[len], sizeof(finalOutput)-len, " and %d more rounds", remainingrounds);
				}
			}
		}
	}
	...
	if (!lastround)
	{
		...
		if (!maxroundscontrolled && g_Cvar_MaxRounds)
		{
			int maxrounds = g_Cvar_MaxRounds.IntValue;
